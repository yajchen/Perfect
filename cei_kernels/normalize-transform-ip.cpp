...
void serial_transform(uint8 **pInp,uint16 mean,uint16 std){
...
for(int x=0;x<x_dim;x++){
  ...
  pInp[x][y] = (tmp<=255)? ((uint8)tmp) : 255;
  }
}
...
void column_transform(uint8 **pInp,uint32 col,uint16 mean,uint16 std){
float sigma_ratio = ((float)std_desgin)/((float)std);
float tmp;
for(int x=0;x<x_dim;x++){
    tmp=(pInp[x][col]-mean)*sigma_ratio+mean_design;
    pInp[x][col] = (tmp<=255)? ((uint8)tmp) : 255; 
}
...
serial_transform(pInp,mean,std);
...
ar::simt_tau::par_for(y_dim, [&](size_t col) {
     column_transform(pInp,col,mean,std);
});
gettimeofday(&tim4, NULL); 
gem5::DumpStats();
