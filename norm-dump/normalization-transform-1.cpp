...
void column_transform(uint8 **pInp,uint8 **pOut,uint32 tid,uint16 mean,uint16 std){
float sigma_ratio = ((float)std_desgin)/((float)std);
float tmp;
for(int x=0;x<x_dim;x++){
  for(int s=0;s<stride;s++){
    tmp=(pInp[x][tid*stride+s]-mean)*sigma_ratio+mean_design;
    pOut[x][tid*stride+s] = (tmp<=255)? ((uint8)tmp) : 255;
  }
}
...
gem5::DumpStats();
gettimeofday(&tim3, NULL); 
ar::simt_tau::par_for((y_dim/stride), [&](size_t tid) {
     column_transform(pInp,pOut,tid,mean,std);
});
gettimeofday(&tim4, NULL);
gem5::ResetStats();
